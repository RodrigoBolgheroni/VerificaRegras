...
def atualizarStatusArquivo(id_arquivo,ambiente,container,diretorio,nomearquivo,tamanho,quantidade_registros,status,mensagem_erro, id_tabela_pbi):
    """
    ...
    try:
        ...
        query_verificacao = f"""
            SELECT Id FROM [monitor].[tblstatusarquivo] WHERE IdTabelaPBI = {id_tabela_pbi}
        """
        ...
        if resultado:
            ...
            query = f"""
                INSERT INTO [monitor].[tblstatusarquivo] (
                    ...
                    {id_arquivo},'{ambiente}','{container}','{diretorio}','{nomearquivo}',{tamanho if tamanho else ""},{quantidade_registros if quantidade_registros else ""}, '{status}','{mensagem_erro if mensagem_erro else ""}','{data_hora_str}',{id_tabela_pbi}, 
                     '{data_hora_str}'
                )
            """
            conn.execute(text(query))
...
def selecionaIdLog():
    conn = conecta_sqlserver()        
    resultado = conn.execute(text("SELECT COALESCE(MAX(IdProcessamento),0) AS IdProcessamento  FROM [monitor].[tbllog]")).fetchone()
